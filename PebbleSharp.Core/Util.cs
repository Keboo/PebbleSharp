...
namespace PebbleSharp.Core
{
    internal static class Util
    {
        ...
        public static byte[] GetBytes( string @string, bool prefixWithLength = true )
        {
            ...
            var bytes = new byte[@string.Length + ( prefixWithLength ? 1 : 0 )];
            if ( prefixWithLength )
                bytes[0] = (byte)@string.Length;
            Encoding.UTF8.GetBytes( @string, 0, @string.Length, bytes, ( prefixWithLength ? 1 : 0 ) );
            return bytes;
        }
